...
from Steward.utils.viewUitils import get_character_sections, get_character_select_option
...
class PlayerInfoView(BaseInfoView):
    def __init__(self, bot: StewardBot, ctx: StewardContext, player: Player, **kwargs):
        ...
        if self.player.active_characters:
            container.items.extend(get_character_sections(self.player, self.ctx.server, self._on_char_swap))
...
class CharacterInfoView(BaseInfoView):
    ...
    def _build_content(self) -> list:
        ...
        container = ui.Container(
            ui.Section(
                ui.TextDisplay(
                    f"{self.character.name} ({self.player.mention}){'*' if self.character.primary_character == True else ''}\n"
    ...
    def _add_activity_section(self, container: ui.Container):
        if self.ctx.server.activity_points:
            activity_point = self.ctx.server.get_activity_for_points(self.character.activity_points)
